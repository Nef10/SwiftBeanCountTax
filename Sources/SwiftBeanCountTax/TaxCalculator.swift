...
public enum TaxCalculator {
    ...
    public static func generateTaxSlips(from ledger: Ledger, for year: Int) throws -> [TaxSlip] {
        ...
        return try slips.flatMap { slipName throws in
            let slip = slipName.replacingOccurrences(of: "(", with: "").replacingOccurrences(of: ")", with: "")
            ...
            return try getTaxSlips(slip, name: slipName, year: year, commodity: commodity, splitAccounts: splitAccounts, taxYearTransactions: taxYearTransactions, ledger: ledger)
        }
